...
#                        'sheep', 'sofa', 'train', 'tvmonitor'])
  ...
  if webcam_num >= 0 :
    ...
    imglist = [f for f in os.listdir(args.image_dir) if not f.startswith('.')]
  ...
  while (num_images >= 0):
      ...
      predicts_path = os.path.join("predicts", imglist[num_images][:imglist[num_images].find(".")] + "_" + args.dataset + ".txt")
      f = open(predicts_path, mode='w')
      ...
      for j in xrange(1, len(pascal_classes)):
          ...
          if inds.numel() > 0:
            ...
            for k in range(cls_dets.shape[0]):
              f.write(" ".join(list(map(str, cls_dets[k].numpy().tolist()))) + " " + pascal_classes[j] + "\n")
      f.close()
