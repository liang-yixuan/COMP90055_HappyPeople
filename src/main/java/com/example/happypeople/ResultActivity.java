...
public class ResultActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable @org.jetbrains.annotations.Nullable Bundle savedInstanceState) {
        ...
        try {
            uploadImage(getRealPathFromURI(selectedImageUri), url);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        okHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                try {
                    ...
                    JSONArray obj_array = new JSONArray(json.getString("objects"));
                    ArrayList<String> data = new ArrayList<>();
                    for (int i = 0; i < obj_array.length(); i++) {
                        JSONObject obj = obj_array.getJSONObject(i);
                        String result = "Name:    " + obj.getString("name") + "\n";
                        if (obj.has("Age")) {
                            result = result + "Age:     " + obj.getString("Age") + "\n";
                        } else {
                            result = result + "Age:     Unknown\n";
                        }
                        if (obj.has("Gender")) {
                            result = result + "Gender:  " + obj.getString("Gender") + "\n";
                        } else {
                            result = result + "Gender:  Unknown\n";
                        }
                        if (obj.has("Race")) {
                            result = result + "Race:    " + obj.getString("Race") + "\n";
                        } else {
                            result = result + "Race:    Unknown\n";
                        }
                        if (obj.has("Emotion")) {
                            result = result + "Emotion: " + obj.getString("Emotion");
                        } else {
